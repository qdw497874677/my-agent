...
/** Conservative Admin detail formatter for mobile governance cards. */
final class AdminMobileRedactor {

    static final String REDACTED = "[REDACTED]";
    private static final int DEFAULT_MAX_CHARS = 180;
    private static final Pattern KEY_VALUE_SECRET = Pattern.compile(
            "(?i)(rawsecret|api[_-]?key|password|secret|token|authorization|access_token|refresh_token)\\s*=\\s*[^,}\\]\\s|]+"
    );
    private static final Pattern BEARER_SECRET = Pattern.compile("(?i)bearer\\s+[^,}\\]\\s|]+");
    private static final Pattern OPENAI_SECRET = Pattern.compile("(?i)sk-[A-Za-z0-9._:-]*");
    private static final Pattern RAW_SECRET_WORD = Pattern.compile("(?i)rawsecret");

    private AdminMobileRedactor() {
    }
    ...
    static String redact(String value) {
        ...
        redacted = OPENAI_SECRET.matcher(redacted).replaceAll(REDACTED);
        redacted = RAW_SECRET_WORD.matcher(redacted).replaceAll(REDACTED);
        return redacted;
    }
    ...
    private static String stringifyValue(Object value) {
        ...
        if (value instanceof Map<?, ?> map) {
            return map.entrySet().stream()
                    .sorted(Comparator.comparing(entry -> sensitiveKeyRank(entry.getKey()) + ":" + stringifyValue(entry.getKey())))
                    .map(entry -> stringifyValue(entry.getKey()) + "=" + stringifyValue(entry.getValue()))
                    .collect(Collectors.joining(", ", "{", "}"));
        }
        if (value instanceof Collection<?> collection) {
            return collection.stream().map(AdminMobileRedactor::stringifyValue).collect(Collectors.joining(", ", "[", "]"));
        }

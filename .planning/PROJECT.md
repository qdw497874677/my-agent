...
Phase 8 complete — the repository now contains controlled dynamic plugin JAR support with PF4J isolated inside `pi-agent-infrastructure-plugin`, typed controlled-directory configuration, PF4J-to-Pi extension bridging, refreshable/state-aware plugin governance, `DynamicPluginToolRegistry` resolution, public Admin REST/UI governance contracts, and a deterministic sample plugin JAR. No-key product-path E2E and architecture/redaction gates prove plugin tools inherit gateway/policy/audit/event/redaction boundaries, refresh can rediscover controlled-directory plugins, disable/quarantine affect new runs, allowlist/selected controls are enforced, and Adapter Web remains free of direct PF4J imports. Phase 9 can now focus on observability, policy, tenancy, and production hardening.

...
- [x] Phase 18 validated true streaming UI semantics: model deltas aggregate into one live assistant bubble, replay/push/poll events dedupe by event identity or sequence, terminal/cancel/failure states mutate the same bubble safely, Vaadin Push is the Console product streaming path, and no-key Java/Playwright gates cover semantic selectors and Phase 21 handoff. Validated in Phase 18: Streaming Bubble Lifecycle.
- [x] Phase 19 validated selected-session multi-turn runtime context: App-layer context assembly now derives bounded, safe prior user/assistant turns from typed transcripts, dispatch injects them into `RunContext.sessionContext().messages()`, OpenAI-compatible providers receive ordered chat messages with the current prompt appended exactly once, and no-key fake-model/provider gates prove model-visible history without Vaadin-owned context. Validated in Phase 19: Multi-Turn Runtime Context.
- [x] Phase 20 validated provider/model and local profile stability: Console provider/model readiness is visible, model refresh errors are explicit and redacted, model choices persist and snapshot per run, no-provider sends are blocked unless explicit local fallback is active, safe provider/model/fallback metadata flows through run history and dispatch, and same-DB SQLite restart proof recovers provider config, sessions, transcripts, and run metadata. Validated in Phase 20: Provider/Model and Local Profile Stability.

### Active

- [ ] 将 Console 从临时可用对话入口产品化为 Kimi 首页式 Agent 对话体验，默认聚焦输入、回复和历史入口，隐藏未完成运维噪音。
- [ ] 实现历史会话列表、选择、恢复消息记录和基于历史会话继续对话。
- [x] 实现真实流式 UI：模型 delta 实时追加到同一助手气泡，并能正确处理完成、错误和取消状态。Validated in Phase 18: Streaming Bubble Lifecycle.
- [x] 实现 session 级多轮上下文输入，让后续消息能利用当前会话历史。Validated in Phase 19: Multi-Turn Runtime Context.
- [x] 稳定 local profile 对话闭环：页面配置 provider、模型选择、SQLite 持久化、fallback/error 反馈和 focused Java 验证门槛。Validated in Phase 20: Provider/Model and Local Profile Stability.
...
Phase 20 complete — Console provider/model readiness, explicit fallback/no-key semantics, per-run model snapshots, safe provider metadata persistence, and same-DB SQLite local profile restart recovery are verified. Remaining v1.2 product gap is final verification, security, and regression hardening.
...
---
*Last updated: 2026-07-04 after completing Phase 20 Provider/Model and Local Profile Stability*

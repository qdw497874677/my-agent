...
## Current Milestone: v1.1 适配移动端web

**Goal:** 将现有 Vaadin Web Console 与 Admin Governance 从“桌面优先、手机可访问”升级为“移动优先、主流手机浏览器可验证可用”的 H5 体验。

**Target features:**
- 移动优先重构现有 Web UI 信息架构与布局，而不是只补少量响应式样式。
- 全站覆盖：Console、Agent Catalog、Chat/Run、Run Timeline、工具卡、审批卡、Session 历史，以及 Admin Governance/Operations/MCP/Plugin/Extension/Policy/Audit 等已存在页面。
- 支持主流手机浏览器目标：Android/iOS 上的 Chrome、Safari、Edge、Firefox 等移动浏览器视口。
- 增加移动端验收：典型手机/平板断点、触控可用性、横竖屏/窄屏、无水平溢出、关键操作可完成、Playwright mobile viewport/browser smoke。
- 保持现有 Java/Vaadin/public REST/SSE DTO 边界，不引入 React/Next.js 或独立移动 App。
...
- [ ] 为主流手机浏览器和典型移动/平板视口建立可自动运行的响应式、触控和关键路径验证门槛。
...
Phase 14 complete — the v1.1 mobile Web work now has Admin Governance full-site mobile coverage for Overview, Registry, MCP, Plugin, Extension, Operations, Policy Decisions, Audit, and Approval-adjacent Admin routes. Automated Java Admin card/detail contracts and the MVER-04 Playwright Mobile Chrome route list gate passed. The next milestone phase focuses on cross-browser, orientation, accessibility, real-device/UAT, and release hardening.
...
---
*Last updated: 2026-06-25 after completing Phase 14 Admin Governance Full-Site Mobile Coverage*

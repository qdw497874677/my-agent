...
- [ ] 提供扩展集成体系，v1 覆盖 Java SPI、Spring Bean 注册、动态插件 Jar 和远程 MCP 工具接入。Phase 6 delivered Java SPI, Spring Bean, and lightweight annotation extension paths with read-only governance and conformance tests; Remote MCP and dynamic plugin Jar loading remain active for later phases.
...
Phase 6 complete — the repository now contains a framework-free Java extension API/SPI module, ServiceLoader discovery and normalized contribution registry, Spring Boot starter/autoconfiguration, and lightweight `@PiTool`/`@PiEventListener` registration. Adapter Web consumes the same starter path and exposes read-only Admin extension governance through public REST/UI boundaries. No-key conformance tests prove SPI/Spring extensions cannot bypass ToolExecutionGateway, Policy, Audit, Event, redaction, workspace, or credential-reference boundaries. Phase 7 can now build the MCP client bridge on top of the same source/capability/governance language, and Phase 8 can reuse the contracts for dynamic plugin Jar loading.

...
- [x] **PLUG-01**: Admin can configure a controlled plugin directory for trusted dynamic plugin JARs. Validated in Phase 8: `pi.plugins.directory`, `PluginRegistryProperties`, `Pf4jControlledPluginDiscoveryService`, `PluginGovernanceBeanConfiguration`, live refresh in `SamplePluginJarCompatibilityE2ETest`, controlled-directory load in `SamplePluginJarE2ETest`, and `docs/phase-08-controlled-dynamic-plugin-jars.md`.
- [x] **PLUG-02**: Platform can load plugin descriptors, validate platform/API compatibility, and register plugin-provided capabilities through the same extension registry. Validated in Phase 8: `Pf4jControlledPluginDiscoveryService`, `Pf4jPluginSourceDiscovery`, `Pf4jPluginExtensionBridge`, refreshable `PluginGovernanceCatalogAdapter`, sample `plugin.properties`, allowlist/selected coverage in `PluginGovernanceCatalogAdapterTest`, sample compatibility E2E, and expanded `PluginInfrastructureArchitectureTest` Adapter Web PF4J isolation gate.
...
- [x] **OPS-01**: Platform emits structured logs, metrics, and OpenTelemetry-compatible trace/span hooks for runs, model calls, tool calls, MCP calls, plugin lifecycle, and policy decisions. Validated in Phase 9 by `pi-agent-infrastructure-observability`, `RunEventTelemetrySink`, `TelemetryToolExecutionGateway`, `TelemetryStreamingModelClient`, `TelemetryMcpGovernanceCatalog`, `TelemetryPluginGovernanceCatalog`, Admin operations API/UI, `Phase09TelemetryRedactionRegressionTest`, `Phase09CriticalPathRegressionTest`, and `docs/phase-09-production-hardening.md`.
...
- [x] **E2E-08**: Integration E2E verifies Fake MCP server discovery/execution and sample plugin JAR loading/disable flows through the same ToolExecutionGateway, policy, audit, and event pipeline. MCP portion validated in Phase 7 by `McpGovernedToolE2ETest`, `McpSecurityRedactionE2ETest`, and `e2e/phase-07-mcp-governance.spec.ts`; plugin governed execution, live REST refresh, live disable/quarantine, allowlist/selected, and redaction portions validated in Phase 8 by `PluginGovernedToolE2ETest`, `PluginCapabilityDisablementTest`, `PluginSecurityRedactionE2ETest`, `SamplePluginJarE2ETest`, `SamplePluginJarCompatibilityE2ETest`, `DynamicPluginToolRegistry`, and expanded Adapter Web PF4J architecture gates.
...
| PLUG-01 | Phase 8 | Complete — validated by controlled `pi.plugins.directory` configuration, typed `PluginRegistryProperties`, `Pf4jControlledPluginDiscoveryService`, Adapter Web binding, live REST refresh, sample plugin controlled-directory E2E, and Phase 8 operations docs |
| PLUG-02 | Phase 8 | Complete — validated by PF4J descriptor discovery, compatibility checks, bridge into `ExtensionSource`/`DefaultExtensionContributionRegistry`, allowlist/selected contribution filtering, sample plugin packaging, and expanded Adapter Web PF4J architecture gates |
...
| PLUG-05 | Phase 8 | Complete — validated by disable/quarantine REST operations, audited mutation responses, `DynamicPluginToolRegistry` capability removal from new registry list/resolve calls, and live sample plugin disable/quarantine E2E |
...
| OPS-01 | Phase 9 | Complete — validated by `pi-agent-infrastructure-observability`, run/model/tool/policy/MCP/plugin telemetry wrappers, Admin operations API/UI, Phase 9 redaction and critical-path regression tests, and production hardening documentation. |
...
| E2E-08 | Phase 7, Phase 8 | Complete — MCP portion validated in Phase 7 by governed fake MCP discovery/execution/status tests; plugin portion validated in Phase 8 by governed plugin execution/redaction tests plus real sample plugin JAR load, compatibility, refresh, allowlist/selected, disable, and quarantine product-path E2E |

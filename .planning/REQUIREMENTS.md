...
- [ ] **MH5-01**: Mobile user can open every existing Console and Admin Governance route at representative phone viewports without blank screens, route errors, or desktop-only blocking messages.
...
- [ ] **MH5-03**: Mobile user does not encounter page-level horizontal overflow at representative phone, phone landscape, and tablet viewports.
...
- [ ] **MCON-01**: Mobile user can browse Agent Catalog as stacked cards and select/start the General Agent without desktop-width layout.
...
- [ ] **MCON-04**: Mobile user can open session history, select a past session, continue it, and clearly see the active session.
...
- [ ] **MCARD-01**: Mobile user can inspect run timeline events as compact cards or accordions with status, timestamp/type, summary, and expandable details.
...
- [ ] **MCARD-03**: Mobile user can expand dense run/tool/policy/audit details without exposing raw sensitive payloads or causing page-level horizontal overflow.
...
- [ ] **MCARD-05**: Mobile user sees dialogs, drawers, notifications, and confirmations fit the viewport with safe scrolling and explicit close/action controls.
...
- [ ] **MVER-01**: Automated browser tests run representative Mobile Chrome, Mobile Safari/WebKit, Mobile Firefox or Firefox mobile viewport, and tablet contexts where supported by CI.
...
- [ ] **MVER-06**: Desktop Web Console/Admin browser regressions remain passing after mobile-first changes.
- [ ] **MVER-07**: Release documentation records real-device/UAT expectations for Android Chrome, iOS Safari, Edge mobile, and Firefox mobile, including any CI/emulation gaps.

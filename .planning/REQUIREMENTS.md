...
- [x] **PROV-03**: Model selection persists locally and affects only subsequent runs.
- [x] **PROV-04**: Each run records the actual provider, model, and fallback mode used for history and debugging.
- [x] **PROV-05**: No-provider fallback is clearly labeled as local fallback, not a real model answer.
...
- [x] **VER-01**: Automated tests cover no-key fallback, configured-provider path, recent-session restore, same-session continuation, streaming coalescing, cancellation/error states, and provider errors.

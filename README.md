...
| Layer | 职责 | 在 Pi Java 中的例子 |
|---|---|---|
| Adapter | 对外入口和协议适配 | REST/SSE API、Agent Web Console、Admin Governance、future TUI/CLI adapter |
| App | 应用编排和用例执行 | CreateRun、CancelRun、StreamEvents、RegisterTool、LoadPlugin、ConnectMcp |
| Domain | 核心领域模型和业务规则 | AgentRuntime、Run、Step、RunEvent、ToolDescriptor、ToolPolicy、ProviderCapability |
| Infrastructure | 外部系统和技术实现 | PostgreSQL、Spring AI/OpenAI adapter、MCP client、PF4J、OpenTelemetry、Vaadin/Spring wiring |

依赖规则：

```text
Adapter 依赖 App
App 依赖 Domain，并通过 Domain Gateway/Port 使用 Infrastructure
Infrastructure 实现 Domain 定义的 Gateway/Port
Domain 不依赖任何外层
```
...
Core 属于 COLA Domain 层，只定义领域模型、状态机、Gateway/Port 和事件协议。

### 4. Adapter Containment

Spring Boot、Spring AI、MCP Java SDK、PF4J、Vaadin、PostgreSQL、OpenTelemetry 都属于 Adapter/Infrastructure，不允许反向定义平台核心模型。
...
| Architecture | COLA: Adapter → App → Domain ← Infrastructure |
...
| Testing | JUnit, AssertJ, Testcontainers, fake providers/tools, ArchUnit, headless E2E, browser E2E |
...
Pi Java 的 E2E 验证围绕 **Run 生命周期**，而不是围绕单个页面或单个工具。每条关键 E2E 都要证明：用户或系统发起一次 Agent Run 后，Runtime、Model、Tool、Policy、Audit、Event、Persistence、SSE、Web Console 能形成可信闭环。

标准 E2E 链路：

```text
API / Agent Web Console
  → Cloud Server
    → Agent Runtime
      → Model Provider
      → ToolExecutionGateway
  ...
  → Persistence query confirms state
```
...
| Layer | 目标 | 典型工具 |
|---|---|---|
| Headless E2E | 不启动浏览器，验证 API + Runtime + DB + SSE | JUnit, SpringBootTest, Testcontainers, fake model/tool, SSE client |
| Contract E2E | 验证 API 和 RunEvent 协议稳定 | OpenAPI, JSON schema/snapshot, serialization tests |
| Browser E2E | 验证真实 Web Console 用户路径 | Playwright / Selenium / Vaadin TestBench |
| External Integration E2E | 验证模拟外部系统集成 | Fake OpenAI-compatible server, Fake MCP server, sample plugin JAR |

### v1 必须覆盖的 E2E 场景

1. **最小 Agent Run 成功**：创建 Run → FakeModel 输出 → SSE 收到事件 → Run completed → DB 可查询。
2. **Agent 调用工具成功**：FakeModel 输出 ToolCall → ToolGateway 执行 FakeTool → Audit/Event 落库 → 最终回答完成。
3. **Policy 拒绝工具调用**：高风险工具被 DENY → Tool executor 未执行 → policy/audit/event 可查。
4. **工具调用需要审批**：Policy 返回 REQUIRE_APPROVAL → Run 等待审批 → Web/API 批准 → Tool 执行 → Run completed。
5. **取消运行**：Run 运行中取消 → model/tool 传播取消 → Run cancelled → SSE terminal event → DB 状态一致。
6. **SSE 断线重连**：Client 断线后通过 lastEventId/sequence 恢复，不丢终态事件。
7. **Secret 不泄漏**：CredentialRef、tool input/output、logs、events、audit、Web Console 默认不显示 raw secret。
8. **MCP 工具调用**：Fake MCP server tools/list → ToolDescriptor 归一化 → MCP tool 经 ToolGateway 调用。
9. **动态插件工具调用**：Sample plugin JAR 加载 → 注册 tool → 调用经过 ToolGateway → disable/quarantine 后不可用。

### 推荐测试模块

```text
pi-testkit      # FakeModelProvider, FakeTool, FakePolicy, EventCollector, assertions
pi-e2e-tests    # SpringBootTest, Testcontainers, fake external servers, browser smoke tests
```
...
```bash
mvn test                         # unit + arch + contract fast tests
mvn verify -P integration         # integration tests with Testcontainers
mvn verify -P e2e                 # headless E2E
...
```text
用户打开 Agent Web Console
→ 选择一个 Agent
→ 输入任务或 Chat 消息
→ Cloud Server 创建 Run
→ SSE 推送 RunEvents
→ Model 输出 ToolCall
→ ToolGateway 校验 schema
→ Policy 允许/拒绝/要求审批
→ Web Console 展示 Tool Card / Approval Card
→ Tool 执行并写入 Audit
→ Model 生成最终结果
→ Run completed
→ 用户可在 Session history 中继续
→ Admin 可看到 Provider/Tool/Policy/Audit/Plugin/MCP 状态
```

...
项目的第一阶段首要交付形态是 **Cloud Server**：一个可部署、可观测、可扩展、可治理的 Java 云上 Agent 服务，提供 Runtime、模型接入、工具调用、插件扩展、Run/Session 管理、REST/SSE API 和最小 Admin GUI。
...
- **Admin GUI**：面向管理员和运维人员的运行时 cockpit，用于查看 Run、事件流、工具调用、插件状态、MCP 连接和模型配置。
...
第一阶段聚焦 **Cloud Server + General Agent + Admin GUI + 扩展集成**。
...
REST/SSE、Admin GUI、未来 TUI/CLI、审计、Trace、Replay 都应该消费同一套事件语义。
...
Admin GUI 和未来 TUI/CLI 使用同一套 REST/SSE/read-model API。
...
| 5 | Admin GUI Runtime Cockpit | 提供 all-Java 运行时 cockpit，用于查看 Run、事件、工具、Provider、扩展状态 |
